...
Event schema 10 records `actor_player_id` on every player-generated event and stores a vector of player physics outcomes in canonical player-ID order. Automatic oxygen and incapacitation payloads carry an explicit target player ID while retaining the system envelope with no actor. Idempotency is scoped by `(actor_player_id, operation_id)`, preventing one player's chosen operation ID from replaying another player's receipt. Replay verifies that an actor or automatic lifecycle transition could affect only its canonical player's controls, suit, respawn, carried inventory, range origin, environment, and career credit at the prior event state.

All living players occupy the same Jolt scene and are advanced in the same fixed-step event as grids. P1.0 deliberately disables player-to-player collision using collision layers while retaining player collision against planets, voxels, and grids; character collision and pushing remain a later evidence checkpoint. Each player's locomotion probes ignore only that player's own body. Mining, building, welding, and hand-tool damage reconstruct the bound actor's canonical eye ray, intersect voxel cells and rotated grid blocks, and require the requested identity or build face to be the stable closest visible surface within the inclusive nine-metre reach. The rule runs during preparation and replay; client rays and outcomes are not trusted. Shared-grid commands require a separate permission decision and are initially denied except for the existing development owner.

Complete JSON rosters remain the P1.0 correctness transport. The worker retains one latest complete structural snapshot and one latest motion snapshot for the cell, independent of subscriber count. A connection drains at most one retained state message per 60 Hz replication period and skipped periods do not accumulate. Superseded motion is discarded; required structural state is delivered first, with newer motion following on a later period. Because every structural message is a complete snapshot, a newer structural snapshot subsumes any unsent older one without losing a world mutation. Initial handshakes, explicit snapshot requests, and structural mutations receive complete snapshots; motion lag alone never triggers a complete-snapshot recovery loop. Interest-managed binary deltas are the next P1 transport slice and must preserve the same actor, ordering, and snapshot semantics.
