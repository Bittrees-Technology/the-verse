...
3. `J` toggles the authoritative jetpack state. The server owns inverse-square gravity, EVA motion, rotation, and ballistic landing/contact under [ADR-0013](../decisions/ADR-0013-input-only-authoritative-character-motion.md); the client predicts those values for presentation and never submits a transform. Jetpack drift accumulates gravity with dampeners off, powered dampeners compensate while idle, and jetpack-off translation input supplies no walking force.
...
Khepri Prime is a bounded planetary-environment and spherical-collision proof around the origin cell. Its visible surface is not yet a globally streamed editable voxel sphere, and its decorative terrain scatter has no canonical resource yield. P0.9 supplies server-authoritative input-based EVA and landing contact through a bounded sphere controller; native prediction remains presentation only. Walking, jump, slopes, stairs, ladders, magnetic boots, and moving-platform attachment remain P0.10-or-later work. Full planet streaming, weather, pressurized rooms, air vents, airtightness graphs, oxygen tanks, general combat/impact health, and grid gravity generators remain later checkpoints.
