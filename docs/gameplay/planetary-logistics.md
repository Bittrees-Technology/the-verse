...
3. `J` toggles the authoritative jetpack state. The current client predicts the same gravity vector in both modes: jetpack drift accumulates gravity with dampeners off, powered dampeners compensate it when idle, and the provisional jetpack-off client path predicts walking, jumping, and surface contact. P0.9 replaces accepted positions with server-owned EVA, ballistic landing/contact, and rotation under [ADR-0013](../decisions/ADR-0013-input-only-authoritative-character-motion.md); it deliberately does not make the provisional walking or jump behavior authoritative.
...
Khepri Prime is a bounded planetary-environment proof around the origin cell. Its visible surface is not yet a globally streamed editable voxel sphere, its decorative terrain scatter has no canonical resource yield, and the player controller is predicted rather than a production character-body solver. Gravity-aware jetpack behavior is a client prediction improvement until P0.9 supplies server-authoritative input-based EVA and landing contact. Walking, jump, slopes, stairs, and moving-platform attachment remain P0.10-or-later work. Full planet streaming, weather, pressurized rooms, air vents, airtightness graphs, oxygen tanks, general combat/impact health, grid gravity generators, and rigid-body ground collisions remain later checkpoints.
