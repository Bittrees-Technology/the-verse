...
11. Inventory range checks use only canonical server state. Mining, welding, building, and damaging additionally reconstruct the authenticated actor's canonical eye ray and require the requested voxel, block, or build face to be the single closest visible usable surface within the inclusive nine-metre range.
