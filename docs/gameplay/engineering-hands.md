...
1. Construction mode snaps a preview to the exact outward face of the single closest visible block selected by the canonical eye ray.
...
1. The server reconstructs the authenticated actor's eye ray and validates that the selected integer voxel is the single closest visible usable surface within nine metres.
...
- Occluded mining, welding, damage, and incorrect-face construction are rejected without mutation during both live preparation and replay.
...
The complete interaction authority and tie rules are specified in [Authoritative hand-tool targeting](authoritative-hand-tool-targeting.md).
